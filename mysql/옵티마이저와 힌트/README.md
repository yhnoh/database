...
- use_index_extensions 옵티마이저 옵션은 ***세컨더리 인덱스에 자동으로 추가된 프라이머리 키를 활용***할 수 있게 할지를 결정하는 옵션이다.
- 클러스터링 인덱스에서 확인한 것 처럼, InnoDB 스토리지 엔진은 프라이머리 키를 클러스터링 키로 생성한다.
  - 때문에 ***모든 세컨더리 인덱스는 리프 노드에 프라이머리 키 값을 가진다.***
- 세컨더리 인덱스의 리프 노드가 프라이머리 키 값을 가지기 때문에 `(세컨더리 인덱스, 프라이머리 인덱스)`를 생성한 것과 흡사하게 동작할 수 있다.
- `dept_no` 테이블에서 `(dept_no, emp_no)` 프라이머리 키와 `from_date` 세컨더리 인덱스를 가질 경우, 각 키를 통하여 조회했을 때 실행 계획을 먼저 확인해보자.
```sql
-- 프라이머리 키를 통한 조회
explain
...
+--+-----------+--------+----------+----+-------------------+-------+-------+-----+-----+--------+-----------+
|1 |SIMPLE     |dept_emp|null      |ref |PRIMARY,ix_fromdate|PRIMARY|16     |const|38016|100     |Using index|
+--+-----------+--------+----------+----+-------------------+-------+-------+-----+-----+--------+-----------+
...
|id|select_type|table   |partitions|type|possible_keys|key        |key_len|ref  |rows|filtered|Extra      |
+--+-----------+--------+----------+----+-------------+-----------+-------+-----+----+--------+-----------+
|1 |SIMPLE     |dept_emp|null      |ref |ix_fromdate  |ix_fromdate|3      |const|64  |100     |Using index|
+--+-----------+--------+----------+----+-------------+-----------+-------+-----+----+--------+-----------+
```

- 프라이머리 키를 통해서 조회했을 때 `key_len` 컬럼이 16바이트, 세컨더리 인덱스르 통해서 조회했을 때 `key_len`은 3바이트이다.
> `key_len` 컬럼의 값은 쿼리를 처리하기 위해 인덱스에서 몇개의 컬럼을 사용했는지 알려준다. 정확하게는 인덱스의 각 레코드에서 몇 바이트까지 사용했는지 알려준다.
- 만약 프라이머리 키와 세컨더리 인덱스 둘다에 조건문을 걸었을 경우, 어떤 인덱스를 활용하게 될까?
```sql
-- 프라이머리 키와 세컨더리 인덱스 둘다 사용
explain
...
|id|select_type|table   |partitions|type|possible_keys      |key        |key_len|ref        |rows|filtered|Extra      |
+--+-----------+--------+----------+----+-------------------+-----------+-------+-----------+----+--------+-----------+
|1 |SIMPLE     |dept_emp|null      |ref |PRIMARY,ix_fromdate|ix_fromdate|19     |const,const|6   |100     |Using index|
+--+-----------+--------+----------+----+-------------------+-----------+-------+-----------+----+--------+-----------+
```
- `key_len`의 결과가 19바이트가 나오게 된다. 이렇게 나온 이유는 `(from_date, dept_no, emp_no)`처럼 옵티마이저가 실행 계획을 수립하였기 때문이다.
  - 즉 세컨더리 인덱스와 프라이머리 키를 둘다 활용한다.
- 뿐만 아니라 프라이머리 키가 세컨더리 인덱스에 포함되어 있으므로 정렬 작업에서도 활용 가능하다는 장점이 있다.
...
- 대부분의 옵티마이저는 테이블별로 하나의 인덱스만 사용하도록 실행계획을 수립한다.
...
- `index_merge_intersection` 인덱스 머지 방식은 쿼리를 실행할 때 하나의 인덱스가 아닌 ***여러 인덱스를 각각 검색해서 그 결과의 교집합만 반환***하게 된다.
- 아래 예제를 통해서 한번 확인해보자.
```sql
select * -- 69 건
from employees
where first_name = 'Prasadram' -- 242 건
and emp_no between 10000 and 100000; -- 90000만 건
...
- `Extra`컬럼에서 `Using intersect`라고 표시된 것은 여러개의 인덱스를 각각 검색해서 그 결과의 교집합만 반환했다는 의미이다. 
- 만약 인덱스 머지 실행 계획을 사용하지 않았다면 어떻게 됬을까?
  1. first_name 인덱스만 사용했다면 242건을 검색한 emp_no 컬럼의 조건에 일치하는 레코들들만 반환하는 형태로 처리돼야 한다.
  2. emp_no 조건만 인덱스를 사용했다면 90000만 건의 데이터를 읽어와 first_name 조건에 일치하는 레코드만 반환하는 형태로 처리된다.
- 두 조건을 만족하는 레코드 건수는 69건 이다.
  - ix_firstname 인덱스를 사용했다면 242건의 데이터 페이지 일기를 하지만 그중에서 유효한건 69건이고
  - PRIMARY 키만 사용했다면 10000건을 읽어서 69건만 반환하는 작업이 됏을 것이다.
  - 때문에 각 인덱스를 검색해 두 결과의 교집합만 반환한 것이다.


#### 인덱스 머지 - 합집합 (index_merge_union)

- 인덱스 머지의 Using Union은 where절에 사용된 2개 이상의 조건이 각각의 인덱스를 사용하되 OR 연산자로 연결된 경우에 사용되는 최적화이다.
- Union 알고리즘에서는 A 조건과 B조건과 A와 B 조건을 둘다 만족한는 레코드가 나올 수 있다.
  - 때문에 중복된 레코드가 나올 수 있으므로 제거해 줘야한다.
  - MySQL 서버는 두 결과 집합을 정렬해서 중복 레코드를 제거했다는 의미인데 실제 정렬에 관한 표시는 없다ㅏ
- MySQL은 인덱스의 리프 노드에 프라이머리 키 정보를 가지고 있으며 A와 B를 A의 프라이머리 키와 B의 프라이머리 키를 각각 가져와 서로 비교하면서 중복된 레코드들을 정렬 없이 걸러낼 수 있다.

#### 인덱스 머지 - 정렬 후 집합 (index_merge_sort_union)


- 
- 두 개의 인덱스를 통해서 동일한 결과를 가지고 올 때  