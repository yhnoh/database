...
- 주로 다중 컬럼 인덱스가 걸려 있을 때 선두 컬럼을 활용하지 않으며, 쿼리의 결과가 인덱스에 존재하는 컬럼으로만 처리가 가능한 경우 인덱스 풀 스캔이 일어난다.
```sql
...
-- gender, birth_date 인덱스가 걸려있기 때문에 아래와 같은 쿼리가 활용 가능하다.
...
- 인덱스 풀 스캔보다 좀 더 나은 방식을 제공하기 때문에
- 단점은 존재한다.
