...
- 데이터베이스의 테이블에서 데이터를 검색해서 내가 원하는 결과를 가져 오면 불필요한 디스크 I/O가 필요하기 때문에 느리다.
  - 즉 불필요한 디스크 I/O를 줄이면 조회성능을 끌어 올릴 수 있다는 의미이다. 즉, 내가 필요한 데이터만 I/O하는 것이 가장 효율적인 방법이 될 수 있다. 
- 인덱스를 이용하면 필요한 데이터를 효율적으로 접근하여, 조회 성능을 끌어올리 수 있다.
  - 그렇기 대문에 데이터베이스의 성능 튜닝은 대부분 인덱스의 튜닝으로 해결할 수 있다.
