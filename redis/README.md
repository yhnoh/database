...
때문에 비동기 키 삭제 명령어(`UNLINK`, `FLUSHDB AYSNC`)를 사용하여 메인 스레드의 블로킹을 방지하거나 `redis.conf` 파일에서  `lazyfree-lazy-user-del yes`, `lazyfree-lazy-server-del yes`, `slave-lazy-flush yes` 옵션을 활성화하여 비동기적으로 키를 삭제하는 것이 좋다.
